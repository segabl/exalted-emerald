...
u16 MonTryLearningNewMove(struct Pokemon *mon, bool8 firstMove, bool8 evolved)
{
    ...
    if (firstMove)
    {
        ...
        && (!evolved || gLevelUpLearnsets[species][sLearningMoveTableID].level > 0))
        {
...
u8 GetItemEffectParamOffset(u16 itemId, u8 effectByte, u8 effectBit)
{
    ...
    itemEffect = gItemEffectTable[itemId - FIRST_MEDICINE_INDEX];
    if (!itemEffect)
        return 0;
...
static void GetMoveRelearnerSpeciesMoves(u16 species, u16 *moves, u16 *knownMoves, u8 *numMoves)
{
    u16 i, j;

    for (i = 1; i < NUM_SPECIES; i++)
    {
        for (j = 0; j < EVOS_PER_MON && gEvolutionTable[i][j].targetSpecies != species; j++)
            ;

        if (j >= EVOS_PER_MON)
            continue;

        // if this is a preevolution, collect those moves first
        GetMoveRelearnerSpeciesMoves(i, moves, knownMoves, numMoves);
        break;
    }

    for (i = 0; *numMoves < MAX_MON_MOVES_LEVELUP && gLevelUpLearnsets[species][i].move != LEVEL_UP_END; i++)
    {
        for (j = 0; j < MAX_MON_MOVES && knownMoves[j] != gLevelUpLearnsets[species][i].move; j++)
            ;
...
u8 GetMoveRelearnerMoves(struct Pokemon *mon, u16 *moves)
{
    ...
    GetMoveRelearnerSpeciesMoves(species, moves, knownMoves, &numMoves);
...
u8 GetLevelUpMovesBySpecies(u16 species, u16 *moves)
{
    ...
    for (i = 0; i < MAX_MON_MOVES_LEVELUP && gLevelUpLearnsets[species][i].move != LEVEL_UP_END; i++)
         moves[numMoves++] = gLevelUpLearnsets[species][i].move;
...
{
    u16 moves[MAX_MON_MOVES_LEVELUP];
