...
static void InheritIVs(struct Pokemon *egg, struct DayCare *daycare)
{
    ...
    u8 whichParents[NUM_STATS];
    u8 iv;
    u8 inheritedIvCount;

    // Check if a parent is holding the destiny knot
    inheritedIvCount = INHERITED_IV_COUNT;
    for (i = 0; i < DAYCARE_MON_COUNT; i++)
    {
        if (GetBoxMonData(&daycare->mons[i].mon, MON_DATA_HELD_ITEM) == ITEM_DESTINY_KNOT)
        {
            inheritedIvCount = INHERITED_IV_COUNT_DESTINY_KNOT;
            break;
        }
    }
    ...
    for (i = 0; i < inheritedIvCount; i++)
    {
        ...
        u8 randomIvIndex = Random() % (NUM_STATS - i);
        selectedIvs[i] = availableIVs[randomIvIndex];
        RemoveIVIndexFromList(availableIVs, randomIvIndex);
    }
...
static void BuildEggMoveset(struct Pokemon *egg, struct BoxPokemon *father, struct BoxPokemon *mother)
{
    ...
    // Pass down egg moves from the parents
    numEggMoves = GetEggMoves(egg, sHatchedEggEggMoves);
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        if (sHatchedEggFatherMoves[i] != MOVE_NONE || sHatchedEggMotherMoves[i] != MOVE_NONE)
        {
            for (j = 0; j < numEggMoves; j++)
            {
                ...
                else if (sHatchedEggMotherMoves[i] == sHatchedEggEggMoves[j])
                {
